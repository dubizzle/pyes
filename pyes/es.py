...
class ES(object):
    ...
    def _send_request(self, method, path, body=None, params=None, headers=None):
        ...
        if body:
            ...
            body = ""
        log.error('PYES: url: %s, parameters: %s, headers: %s, body: %s', path, params, headers, body)
        ...
        if response.status != 200:
            log.error('PYES: response content %s %s %s', response.status,response.headers,response.body)
